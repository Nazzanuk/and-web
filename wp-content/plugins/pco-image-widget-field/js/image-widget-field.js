...
function pcoImageWidget($) {
	pcoImage = {
		...
		frame: function(button) {
			...
			if ( button._frame )
				return button._frame;

			// Initialize the frame
			button._frame = wp.media({
				title: data.title,
			...
			// On specific events
			button._frame.on('open', this.updateFrame);
			button._frame.state('library').on('select', this.select);
